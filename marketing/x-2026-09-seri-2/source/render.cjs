...
async function main() {
  ...
  for (const name of fs.readdirSync(__dirname).filter(name => name.endsWith('.svg'))) {
    ...
    const width = Number(svg.toString().match(/width="(\d+)"/)[1]);
    const large = new Resvg(svg, {
      fitTo: { mode: 'width', value: width * 4 },
      font: { fontFiles, loadSystemFonts: true, defaultFontFamily: 'Helvetica Neue' },
    }).render().asPng();
    for (const scale of [1, 2]) {
      await sharp(large).resize({ width: width * scale, kernel: 'lanczos3' }).png({ compressionLevel: 9 }).toFile(path.join(out, scale === 1 ? '' : 'png-2x', name.replace(/\.svg$/, '.png')));
    }
    console.log(name);
  }
}
