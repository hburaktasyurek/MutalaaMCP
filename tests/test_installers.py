...
manifest_spec = importlib.util.spec_from_file_location(
    "build_update_manifest", ROOT / "scripts/build_update_manifest.py"
)
...
def test_update_manifest_binds_the_wheel_and_locked_dependencies(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from mutalaamcp.update import _parse_update_offer
    ...
    offer = _parse_update_offer(json.loads(output.read_text(encoding="utf-8")))
    assert offer.version == __version__
    assert offer.manifest.package.url == (
        "https://github.com/hburaktasyurek/MutalaaMCP/releases/download/"
        f"v{__version__}/{wheel.name}"
    )
    assert offer.manifest.package.hashes == (
    ...
    with pytest.raises(SystemExit):
        manifest_builder.build(wheel, tmp_path / "other.json", "v9.9.9")


def test_update_manifest_export_parses_continuations_markers_and_hashes(
    ...
    dependencies = manifest_builder._export_dependencies()

    assert dependencies == [
        {"name": "aiofile", "version": "3.12.3", "hashes": ["a" * 64, "b" * 64]},
        {
            "name": "pywin32",
            "version": "312",
            "hashes": ["c" * 64],
    ...
    with pytest.raises(SystemExit):
        manifest_builder._export_dependencies()
